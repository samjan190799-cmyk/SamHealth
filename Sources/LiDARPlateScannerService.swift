...
public final class LiDARPlateScannerService: NSObject, ObservableObject {
    ...
    public func startLiveDepthEstimation() {
        ...
        // Fallback: симуляция / оптическая оценка дистанции для устройств без ARKit LiDAR
    ...
    private func startFallbackOpticalEstimator() {
        // Устройства без LiDAR используют оптический дальномер фокуса
...
extension LiDARPlateScannerService: ARSessionDelegate {
    nonisolated public func session(_ session: ARSession, didUpdate frame: ARFrame) {
        Task { @MainActor in
            ...
            var distance: Float = 0.40
            var volumeCm3: Double = 350.0
            var confidence: Float = 0.80
            var status = "Анализ плоскости тарелки..."
            ...
            if #available(iOS 14.0, *), let sceneDepth = frame.smoothedSceneDepth ?? frame.sceneDepth {
                ...
                if let baseAddress = CVPixelBufferGetBaseAddress(depthPixelBuffer) {
                    ...
                    // Замеряем центральную зону тарелки (30% x 30% центральных пикселей)
                    ...
                    if sampleCount > 0 {
                        ...
                        // Расчет реального физического объема:
                        // Физический размер поля зрения на дистанции D: W = 2 * D * tan(FOV/2)
                        let fovFactor = 0.65 // примерный угол охвата камеры
                        let physicalWidthCm = Double(distance) * fovFactor * 100.0 * 0.35
                        let physicalAreaCm2 = physicalWidthCm * physicalWidthCm * 0.785 // площадь круглой тарелки
                        
                        // Толщина рельефа еды над плоскостью тарелки (в среднем 2.5 - 4.5 см)
                        let reliefHeightCm = max(1.5, min(5.5, 3.2 * Double(1.0 - (distance - 0.25))))
                        volumeCm3 = physicalAreaCm2 * reliefHeightCm
                        
                        status = "LiDAR 3D Захват: \(String(format: "%.2f", distance)) м • ~\(Int(volumeCm3)) см³"
                    }
                }
            } else {
                status = targetLocked ? "AR Дальномер: \(String(format: "%.2f", distance)) м" : "Наведите камеру на тарелку (30-50 см)"
            }
            
            // Расчет примерного веса на основе плотности готовой еды (~0.85 - 1.05 г/см³)
            let estimatedGrams = min(1200.0, max(50.0, volumeCm3 * 0.95))
            ...
            self.targetLockDetected = targetLocked
        }
    }
}
#endif
